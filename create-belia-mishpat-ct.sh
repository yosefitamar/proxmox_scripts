...
# Essa CT e o HOST de provisionamento dos containers dos clientes.
#
# O que e instalado:
...
for c in json.load(sys.stdin): print(c.get('vmid',''))" 2>/dev/null
...
list_templates() {
  local node
  node=$(get_node)
  [[ -z "$node" ]] && { warn "Nao foi possivel detectar o node."; return; }
...
select_storage() {
  ...
  if (( ${#storages[@]} == 0 )); then
    die "Nenhum storage disponivel para rootdir. Verifique a configuracao do Proxmox."
  fi
...
provision_ct() {
  ...
  # ============================================================================
  # NOTA: O heredoc << PROVISION (sem aspas) expande variaveis da shell externa.
  # Variaveis que DEVEM ser expandidas: ${pg_host}, ${pg_port}, ${pg_user},
  #   ${pg_password}, ${api_key}, ${api_port}
  # Variaveis que devem permanecer LITERAIS na CT usam \$ para escapar.
  # ============================================================================
...
# ------------------------------------------------------------------------------
echo "[6/7] Criando estrutura de diretorios e scripts..."
...
# Variaveis substituidas via envsubst: CLIENT_SLUG, CLIENT_PORT,
#   PG_HOST, PG_PORT, PG_USER, PG_PASSWORD, APP_IMAGE, APP_VERSION
services:
  app:
...
#
# --drop-db  tambem remove o banco de dados (IRREVERSIVEL)
